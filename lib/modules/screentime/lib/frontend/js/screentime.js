window.screentime = (options) => {
    ...
    function sendGAEvent(field, time) {
        console.debug('GA Event', field, time);
...
document.addEventListener('DOMContentLoaded', function() {
    if('js_sv_tracking_manager_extended_screentime_scripts_screentime' in window) {
        let screentimeSettings = window.js_sv_tracking_manager_extended_screentime_scripts_screentime;
        let screentimeFields = screentimeSettings.tracked_elements;
        if(screentimeFields.length > 0) {
            console.debug('Starting screentime with settings:', screentimeSettings);
            screentime({
                fields: screentimeFields,
                googleAnalytics: screentimeSettings.google_analytics_enabled == "1",
                reportInterval: screentimeSettings.report_interval,
                percentOnScreen: screentimeSettings.percent_on_screen,
            });
        }
    }
});